...
define(['exports', './Matrix2-fc7e9822', './defaultValue-94c3e563', './RuntimeError-c581ca93', './Transforms-a076dbe6', './ComponentDatatype-4a60b8d6'], (function (exports, Matrix2, defaultValue, RuntimeError, Transforms, ComponentDatatype) { 'use strict';
  ...
  QuadraticRealPolynomial.computeRealRoots = function (a, b, c) {
    ...
    if (a === 0.0) {
      ...
      if (
        ...
        cMagnitude / aMagnitude < ComponentDatatype.CesiumMath.EPSILON14
      ) {
        // c ~= 0.0.
        // 2nd order monomial: a * x^2 = 0.
        return [0.0, 0.0];
        ...
        cMagnitude > aMagnitude &&
        aMagnitude / cMagnitude < ComponentDatatype.CesiumMath.EPSILON14
      ) {
        // a ~= 0.0.
        // Constant function: c = 0.
        return [];
    ...
    const q =
      ...
      addWithCancellationCheck$1(
        b,
        ComponentDatatype.CesiumMath.sign(b) * Math.sqrt(radicand),
  ...
  function original(a3, a2, a1, a0) {
    ...
    if (cubicRoots.length > 0) {
      ...
      if (Math.abs(hSquared) < ComponentDatatype.CesiumMath.EPSILON14) {
        // y^4 + p y^2 + r = 0.
  ...
  QuarticRealPolynomial.computeRealRoots = function (a, b, c, d, e) {
    ...
    if (Math.abs(a) < ComponentDatatype.CesiumMath.EPSILON15) {
  ...
  Ray.clone = function (ray, result) {
    ...
    result.origin = Matrix2.Cartesian3.clone(ray.origin);
    result.direction = Matrix2.Cartesian3.clone(ray.direction);
    return result;
  };
  ...
  Ray.getPoint = function (ray, t, result) {
    ...
    result = Matrix2.Cartesian3.multiplyByScalar(ray.direction, t, result);
    return Matrix2.Cartesian3.add(ray.origin, result, result);
  };
  ...
  IntersectionTests.rayPlane = function (ray, plane, result) {
    ...
    const denominator = Matrix2.Cartesian3.dot(normal, direction);

    if (Math.abs(denominator) < ComponentDatatype.CesiumMath.EPSILON15) {
    ...
    result = Matrix2.Cartesian3.multiplyByScalar(direction, t, result);
    return Matrix2.Cartesian3.add(origin, result, result);
  };

  const scratchEdge0 = new Matrix2.Cartesian3();
  const scratchEdge1 = new Matrix2.Cartesian3();
  const scratchPVec = new Matrix2.Cartesian3();
  const scratchTVec = new Matrix2.Cartesian3();
  const scratchQVec = new Matrix2.Cartesian3();
  ...
  IntersectionTests.rayTriangleParametric = function (
    ...
    const edge0 = Matrix2.Cartesian3.subtract(p1, p0, scratchEdge0);
    const edge1 = Matrix2.Cartesian3.subtract(p2, p0, scratchEdge1);

    const p = Matrix2.Cartesian3.cross(direction, edge1, scratchPVec);
    const det = Matrix2.Cartesian3.dot(edge0, p);
    ...
    if (cullBackFaces) {
      if (det < ComponentDatatype.CesiumMath.EPSILON6) {
        return undefined;
      }

      tvec = Matrix2.Cartesian3.subtract(origin, p0, scratchTVec);
      u = Matrix2.Cartesian3.dot(tvec, p);
      if (u < 0.0 || u > det) {
        return undefined;
      }

      q = Matrix2.Cartesian3.cross(tvec, edge0, scratchQVec);

      v = Matrix2.Cartesian3.dot(direction, q);
      if (v < 0.0 || u + v > det) {
        return undefined;
      }

      t = Matrix2.Cartesian3.dot(edge1, q) / det;
    } else {
      if (Math.abs(det) < ComponentDatatype.CesiumMath.EPSILON6) {
        return undefined;
      }
      const invDet = 1.0 / det;

      tvec = Matrix2.Cartesian3.subtract(origin, p0, scratchTVec);
      u = Matrix2.Cartesian3.dot(tvec, p) * invDet;
      if (u < 0.0 || u > 1.0) {
        return undefined;
      }
      ...
      v = Matrix2.Cartesian3.dot(direction, q) * invDet;
      if (v < 0.0 || u + v > 1.0) {
        return undefined;
      }

      t = Matrix2.Cartesian3.dot(edge1, q) * invDet;
    }
  ...
  IntersectionTests.rayTriangle = function (
    ...
    Matrix2.Cartesian3.multiplyByScalar(ray.direction, t, result);
    return Matrix2.Cartesian3.add(ray.origin, result, result);
  };
  ...
  IntersectionTests.lineSegmentTriangle = function (
    ...
    Matrix2.Cartesian3.clone(v0, ray.origin);
    Matrix2.Cartesian3.subtract(v1, v0, ray.direction);
    Matrix2.Cartesian3.normalize(ray.direction, ray.direction);
  ...
  function raySphere(ray, sphere, result) {
    ...
    const diff = Matrix2.Cartesian3.subtract(origin, center, scratchPVec);

    const a = Matrix2.Cartesian3.dot(direction, direction);
    const b = 2.0 * Matrix2.Cartesian3.dot(direction, diff);
    const c = Matrix2.Cartesian3.magnitudeSquared(diff) - radiusSquared;
  ...
  IntersectionTests.lineSegmentSphere = function (p0, p1, sphere, result) {
    ...
    Matrix2.Cartesian3.clone(p0, ray.origin);
    const direction = Matrix2.Cartesian3.subtract(p1, p0, ray.direction);

    const maxT = Matrix2.Cartesian3.magnitude(direction);
    Matrix2.Cartesian3.normalize(direction, direction);
  ...
  const scratchQ = new Matrix2.Cartesian3();
  const scratchW = new Matrix2.Cartesian3();
  ...
  IntersectionTests.rayEllipsoid = function (ray, ellipsoid) {
    ...
    const q2 = Matrix2.Cartesian3.magnitudeSquared(q);
    const qw = Matrix2.Cartesian3.dot(q, w);
    ...
    if (q2 > 1.0) {
      ...
      w2 = Matrix2.Cartesian3.magnitudeSquared(w);
  ...
  function quadraticVectorExpression(A, b, c, x, w) {
    ...
    const l1 =
      ...
      (x *
        addWithCancellationCheck(
          A[Matrix2.Matrix3.COLUMN1ROW0],
          A[Matrix2.Matrix3.COLUMN0ROW1],
          ComponentDatatype.CesiumMath.EPSILON15
        ) +
        b.y);
    const l0 =
      A[Matrix2.Matrix3.COLUMN0ROW0] * xSquared +
      A[Matrix2.Matrix3.COLUMN2ROW2] * wSquared +
      x * b.x +
      c;
    ...
    const r1 =
      wSquared *
      addWithCancellationCheck(
        A[Matrix2.Matrix3.COLUMN2ROW1],
        A[Matrix2.Matrix3.COLUMN1ROW2],
        ComponentDatatype.CesiumMath.EPSILON15
      );
    const r0 =
      w *
    ...
    if (r0 === 0.0 && r1 === 0.0) {
      ...
      solutions.push(new Matrix2.Cartesian3(x, w * cosine0, w * -sine0));
      solutions.push(new Matrix2.Cartesian3(x, w * cosine0, w * sine0));

      if (cosines.length === 2) {
        const cosine1 = cosines[1];
        const sine1 = Math.sqrt(Math.max(1.0 - cosine1 * cosine1, 0.0));
        solutions.push(new Matrix2.Cartesian3(x, w * cosine1, w * -sine1));
        solutions.push(new Matrix2.Cartesian3(x, w * cosine1, w * sine1));
      }
  ...
  const firstAxisScratch = new Matrix2.Cartesian3();
  const secondAxisScratch = new Matrix2.Cartesian3();
  const thirdAxisScratch = new Matrix2.Cartesian3();
  const referenceScratch = new Matrix2.Cartesian3();
  const bCart = new Matrix2.Cartesian3();
  const bScratch = new Matrix2.Matrix3();
  const btScratch = new Matrix2.Matrix3();
  const diScratch = new Matrix2.Matrix3();
  const dScratch = new Matrix2.Matrix3();
  const cScratch = new Matrix2.Matrix3();
  const tempMatrix = new Matrix2.Matrix3();
  const aScratch = new Matrix2.Matrix3();
  const sScratch = new Matrix2.Cartesian3();
  const closestScratch = new Matrix2.Cartesian3();
  const surfPointScratch = new Matrix2.Cartographic();
  ...
  IntersectionTests.grazingAltitudeLocation = function (ray, ellipsoid) {
    ...
    const thirdAxis = Matrix2.Cartesian3.normalize(
      Matrix2.Cartesian3.cross(firstAxis, secondAxis, thirdAxisScratch),
      thirdAxisScratch
    );
    const B = bScratch;
    B[0] = firstAxis.x;
    ...
    const B_T = Matrix2.Matrix3.transpose(B, btScratch);

    // Get the scaling matrix and its inverse.
    const D_I = Matrix2.Matrix3.fromScale(ellipsoid.radii, diScratch);
    const D = Matrix2.Matrix3.fromScale(ellipsoid.oneOverRadii, dScratch);
    ...
    const temp = Matrix2.Matrix3.multiply(
      Matrix2.Matrix3.multiply(B_T, D, tempMatrix),
      C,
      tempMatrix
    );
    const A = Matrix2.Matrix3.multiply(
      Matrix2.Matrix3.multiply(temp, D_I, aScratch),
      B,
      aScratch
    );
    const b = Matrix2.Matrix3.multiplyByVector(temp, position, bCart);

    // Solve for the solutions to the expression in standard form:
    const solutions = quadraticVectorExpression(
      A,
      Matrix2.Cartesian3.negate(b, firstAxisScratch),
      0.0,
      0.0,
      1.0
    );
    ...
    if (length > 0) {
      ...
      for (let i = 0; i < length; ++i) {
        s = Matrix2.Matrix3.multiplyByVector(
          D_I,
          Matrix2.Matrix3.multiplyByVector(B, solutions[i], sScratch),
          sScratch
        );
        const v = Matrix2.Cartesian3.normalize(
          Matrix2.Cartesian3.subtract(s, position, referenceScratch),
          referenceScratch
        );
        const dotProduct = Matrix2.Cartesian3.dot(v, direction);

        if (dotProduct > maximumValue) {
          maximumValue = dotProduct;
          closest = Matrix2.Cartesian3.clone(s, closest);
        }
      }
      ...
      maximumValue = ComponentDatatype.CesiumMath.clamp(maximumValue, 0.0, 1.0);
      altitude =
        Matrix2.Cartesian3.magnitude(
          Matrix2.Cartesian3.subtract(closest, position, referenceScratch)
  ...
  const lineSegmentPlaneDifference = new Matrix2.Cartesian3();
  ...
  IntersectionTests.lineSegmentPlane = function (
    ...
    const difference = Matrix2.Cartesian3.subtract(
      endPoint1,
      endPoint0,
      lineSegmentPlaneDifference
    );
    const normal = plane.normal;
    const nDotDiff = Matrix2.Cartesian3.dot(normal, difference);

    // check if the segment and plane are parallel
    if (Math.abs(nDotDiff) < ComponentDatatype.CesiumMath.EPSILON6) {
      return undefined;
    }

    const nDotP0 = Matrix2.Cartesian3.dot(normal, endPoint0);
    ...
    Matrix2.Cartesian3.multiplyByScalar(difference, t, result);
    Matrix2.Cartesian3.add(endPoint0, result, result);
    return result;
  };
  ...
  IntersectionTests.trianglePlaneIntersection = function (p0, p1, p2, plane) {
    ...
    const p0Behind = Matrix2.Cartesian3.dot(planeNormal, p0) + planeD < 0.0;
    const p1Behind = Matrix2.Cartesian3.dot(planeNormal, p1) + planeD < 0.0;
    const p2Behind = Matrix2.Cartesian3.dot(planeNormal, p2) + planeD < 0.0;
    ...
    if (numBehind === 1 || numBehind === 2) {
      u1 = new Matrix2.Cartesian3();
      u2 = new Matrix2.Cartesian3();
    }
